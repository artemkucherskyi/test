...
from models import SessionLocal, Contact, Invoice, init_db

# Load environment variables from the .env file
load_dotenv()
init_db()  # Ensure tables are created

# Retrieve Odoo configuration from the environment
...
def get_odoo_models():
    """Create an XML-RPC connection to Odoo and return the uid and models proxy."""
...
def sync_contacts(db_session):
    ...
    # Delete any local contacts that no longer exist in Odoo
    db_session.query(Contact).filter(~Contact.odoo_id.in_(odoo_ids)).delete(synchronize_session=False)
    db_session.commit()
    print("Contacts synchronized.")
...
def sync_invoices(db_session):
    ...
    # Delete any local invoices that no longer exist in Odoo
    db_session.query(Invoice).filter(~Invoice.odoo_id.in_(odoo_ids)).delete(synchronize_session=False)
    db_session.commit()
    print("Invoices synchronized.")
